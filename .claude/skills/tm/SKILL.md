...
- NEVER: **現状分析では**、資料に存在しない ID・連番を発明して個体指定子にする。
  個体指定子が見つからないものは**モノではない**。他のモノの属性か、多値か、モデルから外すかのいずれか。
- ただし ID を発明してよいかは**フェーズによって変わる**。「ID が存在しない」は
  「その対象が管理対象と認識されていない」という現状分析の結論であり、**業務改善**
  として管理対象に格上げするなら、ID を新設してモノを立ててよい。
  どちらのフェーズで書いているかを明示し、新設した ID には理由をコメントで残す。
  (例: git のブランチ・ワーキングツリーは名前とパスしか持たないが、アプリが管理する
  ため `GitブランチID` / `ワーキングツリーID` を新設した)
- 逆に、業務改善として書くときは**このアプリが管理しないものはモノにしない**。
  資料に個体指定子があっても、別のエンティティで足りているなら立てず、記録された値は
  右側の属性に置く。(例: 作業ディレクトリ(cwd)は、ディレクトリを Gitリポジトリと
  ワーキングツリーで管理しているので立てず、ログ行・実行中セッションの属性にした)
...
**「ネジは部品か製品か」** — 出典の資料には無いが、分類を左右する重要な観点。
同じモノでも、そのドメインでの扱いによってリソースにもイベントにもなる。

- **部品として扱う** → 単に存在するモノ。リソース
- **製品として扱う** → その**手配(作成・組立・購入)が業務**になる。手配はイベント

つまり「そのモノを自分たちが作る・変えることが機能かどうか」を先に決める。ここを飛ばすと、
本来イベントであるものをリソースにしてしまう(実際にこの判断を落として2回作り直した)。

例: このアプリは `CLAUDE.md` / `settings.json` を**作成・編集するのが機能**なので製品扱いで
イベント。一方セッションログの `.jsonl` は**読むだけ**なので部品扱いでリソース。同じ
「ファイル」でも分類が変わるため、1つのエンティティにまとめない。

なお製品として扱う場合、「作成」と「編集」は別の行為である。1つの箱に `作成日` と
`更新日` を同居させず、**イベントの相違サブセット**(作成 / 更新)に分けるか、DTL
(変更履歴)で持つのが本来の形。
...
| R-R | 対照表 TS(`COMPARATIVE`)。命名は `<モノ>．<モノ>` |
| 再帰 | 再帰表 RC(`RECURSION`)。命名は `<モノ>．<モノ>` |

- 名前に「対照表」「再帰表」は**付けない**。種別は箱の右上のバッジ(`対照` / `再帰`)が
  示すため、名前に入れると重複して横に伸びるだけになる。
- 対照表・対応表は**垂下(`mapping`)で描く**。親2つを1本の線で結び、その中点の○から
  表をぶら下げる。表から親へ2本引いて独立した箱にしない(§2)。
  図の上では親どうしが直結して見えるが、**モデルとしては表を経由している**。
...
- **垂下 `mapping: { entity }`**(0.1.24)。結線の中点に○を置き、そこから対照表・対応表へ
  線を引いてぶら下げる。指定できるのは `COMPARATIVE` / `CORRESPONDENCE` だけで、
  それ以外を指定すると例外で落ちる。**再帰表は対象外**。
  - 多重度は親どうしの1本に集約する。**相手側の端の記号** = 元の「その親 → 表」の
    表側の記号。移すだけで情報は失われない。
  - 垂下線は「線の中点」から「表の上辺または下辺の中央」へ引かれる。きれいな垂線に
    するには**表の中心Xを中点Xに合わせる**。中点の高さに表を置くと線が横に寝る。
  - 垂下線には多重度の記号は描かれない。
