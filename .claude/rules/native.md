...
同一マシンのローカルクライアント(apps/web の Next.js サーバ等)へ、ファイル I/O などの機能を
HTTP で提供する(アプリ = このマシンのプロダクト開発のローカルバックエンド、という役割)。

- MUST: バインドは **127.0.0.1 のみ**。外部インターフェースへ公開しない。
- MUST: 書き込み系エンドポイントは**トークン認証**を必須にする。トークンはアプリ起動時に生成し、
  `app_data_dir` 配下のファイルに保存する(連携先のローカルサーバだけがそのファイルを読む)。
  認証なしの書き込みエンドポイントを作らない。
- MUST: HTTP ハンドラは `#[tauri::command]` と同格の薄い層として扱う(§1 NEVER と同旨):
  引数の変換 → ユースケース(`app`)呼び出し → DTO 化のみ。I/O は `infra`。
- MUST: 書き込み先パスの解決はサーバ側で行う。クライアントから受け取るパスは、
  **登録済みプロファイルの `repository_path` そのもの、またはそこから `git worktree list` で
  引けるworktreeのいずれか**と一致したものだけを起点にできる(それ以外の任意パスは拒否。§4 と同旨)。
  CLAUDE.md の並行作業ルールにより通常の作業は worktree 上で行われるため、`repository_path` との
  完全一致だけを許可すると実運用と噛み合わない(issue #129)。worktree判定は命名規約
  (`.claude/worktrees/` 配下等)に頼らず、必ず実際の `git worktree list` の結果で行う
  (worktree はリポジトリ外にも作られるため)。
- ポートは既定 `14200`(変更可能にする場合は設定へ。ハードコードの散在禁止)。
- エラー・ログにトークンや秘匿情報を含めない(§4)。
