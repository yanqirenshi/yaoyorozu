...
- MUST: 図のレイアウト調整(ノード座標・サイズ等の表示補助情報)は、**レイアウト保存 API** 経由でリポジトリ内のファイル(`src/data/layout/<図名>.json`)に保存し、ページは同 JSON を import して初期レイアウトに使う(git が履歴管理を担う)。この API は「API・DB は導入しない」の**唯一の例外**である。
