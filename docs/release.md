...
ビルドに使う Node のバージョンは、リポジトリ直下の
[`.nvmrc`](../.nvmrc) をソースオブトゥルースとする(現在: 24 系)。
ワークフローは `node-version-file: '.nvmrc'` でこれを読むため、
バージョンを変えるときは `.nvmrc` の1行を書き換える(あわせてルート
`package.json` の `engines.node` も揃える)。公式配布物のビルドは
Active LTS で行う方針のため、上げ先は LTS を選ぶ(issue #68)。
