...
//!
//! セッション間メッセージ(`ListAgents` / `SendMessage` と受信口)は CLI 2.1.280 系から
//! 使える。それより古い版(PATH の 2.1.150 など)には機能自体が無く、一覧に出ず、送っても
//! 「見つからない」になる。app は起動前に版を読み、古ければその旨を状態バー(と一覧項目)に出す
//! (起動は止めない。会話の対話そのものは古い版でも使える)。

/// セッション間メッセージが使える最小の版(`major.minor.patch`)。
pub const MIN_PEER_MESSAGING_VERSION: (u32, u32, u32) = (2, 1, 280);
...
mod tests {
    ...
    #[test]
    fn versions_below_2_1_280_do_not_support_peer_messaging() {
        assert_eq!(
            supports_peer_messaging("2.1.150 (Claude Code)"),
            Some(false)
        );
        assert_eq!(supports_peer_messaging("2.1.279"), Some(false));
        assert_eq!(supports_peer_messaging("1.9.999"), Some(false));
    }

    #[test]
    fn versions_from_2_1_280_support_peer_messaging() {
        assert_eq!(supports_peer_messaging("2.1.280"), Some(true));
        assert_eq!(supports_peer_messaging("2.1.281 (Claude Code)"), Some(true));
    ...
    fn the_warning_names_the_version_and_the_required_one_only_when_unsupported() {
        ...
        assert!(warning.contains("2.1.280"));
        assert_eq!(peer_messaging_warning("2.1.280 (Claude Code)"), None);
    }
}
