...
export const PARTS_ITEMS: IndexItem[] = [
  {
    ...
    label: "製品",
    summary:
      "意味のあるまとまり。ページの内容にあたるデータを取得する層で、フレームが分割した領域に入る。",
  },
  {
    key: "subassembly",
    label: "中間品",
    ...
    body: (
      <>
        <Para>
          ...
          製品はページの内容にあたるデータを取得してよく、中間品と部品は渡された props だけで描画する。
        </Para>
        <Para>
          この境界があるため、中間品と部品は実データがなくても単体で表示でき、/ui
          のカタログにそのまま並べられる。逆に製品は実データを伴うため、カタログには型と作例までを載せる。
        </Para>
        <Note>
          取得を担う層は製品だけではない。フレームも、ナビゲーションやアカウントのような
          「枠を描くためのデータ」を取得する。分かれ目は
          「URL が変わったときに取り直す必要があるか」で、必要ならそれは製品のデータである。
          詳しくは「レイアウト &gt; フレーム」に書いている。
        </Note>
        <Note>
          層の関係(ページ → フレーム → 製品 → 中間品 → 部品)の図は「レイアウト」に置いている。
        </Note>
      </>
    ),
  },
...
export default function PartsIndexPage({
  ...
  return (
    <FoundationPage
      ...
      lead={
        <Para>
          画面の中身にあたるコンポーネント。組み立ての語彙で、製品・中間品・部品の3層に分ける。
