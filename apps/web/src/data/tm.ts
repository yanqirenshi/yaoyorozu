 ...
 *
 * - 複数の実行中セッションを同時に持つことは、既存の「セッション 1 : 実行中セッション
 *   0件以上」と起動元のサブセットで表せている。同時に持てる数の上限は運用の方針で
 *   あって語彙ではないため、モデルには入れない。
 * - 現在のモデル・現在の権限モードを app 起動のサブセットの属性に足した。set_model /
 *   set_permission_mode で途中から変わり、画面に出して切り替えるため(レポート §6.2)。
 *   ログ側の モデルID(AI応答行)・権限モード(ユーザー行)は記録された当時の値で意味が
 *   違うので、別の属性に分けた。更新日時は持たない。変更の履歴はログ側(model_changed
 *   行、ユーザー行の権限モード)に残るため。
 *   未対応: model_changed 行そのものはログ行のサブセットに入れていない(行種別の展開は
 *   第2弾の課題)。
 * - 新規セッションの作成では、セッションIDが起動時に決まる(app が --session-id で決める
 *   こともある)のに対し、会話ファイルは最初の行が書かれた時点で作られる。この間を表せる
 *   よう、セッション → セッションファイル の多重度を「1以上」から「0件以上」に緩めた。
 *   その間の状態は、app 起動サブセットのプロセス状態(起動中)で表す。
 * - どのリポジトリで起動したかは、app 起動のサブセットが リポジトリパス(R) で Gitリポジトリ
 *   と結ぶ。app は起動時にプロファイル(登録済みリポジトリ)を選ぶので、cwd からの推測では
 *   なく記録された事実になる。外部起動は cwd しか無いため従来どおり結ばない。プロファイル
 *   そのもの(対象フォルダ・GitHubプロジェクト等)をモノにするかは、設定の語彙を扱う段で
 *   判断する。
 * - 表示名は既存の語彙で足りる(セッションの会話タイトルと、台帳の名前。Phase 1 の注記の
 *   とおり)。
 * - ハブでの状態表示・起動・停止と、途中経過の配信先(どの画面へ流すか)は画面と実装の
 *   話であり、語彙は増えない。
 *
 * 【Phase 3(worktree の用意)】
 * イシュー #430。app が「このリポジトリの、このブランチの worktree」を用意して、そこを
 * cwd に CLI を起動する段。語彙の要否を判断した記録。
 *
 * - app 起動のサブセットに ワーキングツリーID(R) を足し、ワーキングツリーと結んだ。app が
 *   起動時に worktree を選ぶため、cwd からの推測ではなく記録された事実になる。
 *   ワーキングツリーの個体指定子が ワーキングツリーID + リポジトリパス(R) の組なので、
 *   リポジトリパス(R) は worktree を指す鍵の一部として左側に残る。Phase 2 で張っていた
 *   Gitリポジトリへの線は、worktree から決まる(導出できる)ため外した。git ではリポジトリ
 *   本体のディレクトリも1つ目の worktree なので、worktree を作らずに本体で起動する場合も
 *   この関係で表せる。
 * - ブランチの指定は既存の語彙で足りる。ワーキングツリーと Gitブランチは対応表
 *   (Gitブランチ．ワーキングツリー)で結んであり、「このブランチの worktree」はこの関係を
 *   たどれば決まる。detached HEAD は対応表に行が無い形で表せる。
 * - 役割(デザイン (全体)・実装:APP など。名前・固定ブランチ・表示名・権限モード・モデル)は
 *   モノとして立てない。CLAUDE.md に書かれた運用の方針であって、app が管理する対象では
 *   ないため(#430 でユーザーが判断)。起動時に決まった値は、実行中セッション(app起動)の
 *   既存の語彙(ワーキングツリーID(R)・名前・現在の権限モード・現在のモデル)に記録される。
 *   立てる案(不採用)も残しておく: 役割(リソース)を立て、個体指定子は 役割名 +
 *   リポジトリパス(R)(CLAUDE.md はリポジトリの中にあり、名前だけでは一意にならない)、
 *   属性に 固定ブランチ名・表示名の雛形・既定の権限モード・既定のモデル・担当範囲を持ち、
 *   役割 1 : 実行中セッション(app起動)0件以上 で結ぶ。app が役割の一覧を持って選ばせる
 *   ようになったら、設定(profiles)の語彙と一緒にこの形で立てる。
 * - worktree の作成・最新化の記録は足さない。作成日時・削除日時はワーキングツリーが既に
 *   持っており、最終 merge の時刻などは Git 自身(git worktree list、reflog、マージコミット)
 *   から分かるため、app 側に写し取る必要がない。
 *
 * 【作業ディレクトリを立てない】
 ...
 *   ワーキングツリーID のため結べない。ただし app が起動した実行中セッションだけは別で、
 *   app が起動時に worktree を用意して選ぶため、サブセット側が ワーキングツリーID(R) を
 *   持って結ぶ(Phase 3)。
 ...
 *   77,087行の 97.7% が求まった)。app が起動した実行中セッションは、Phase 2 では
...
const ENTITY_DEFS: EntityDef[] = [
  ...
  // ============ Phase 1(app から claude CLI と対話する)============
  {
    ...
    type: "EVENT-SUBSET",
    description:
      "app が子プロセスとして起動した claude CLI(PoC #382)。区分コードは起動元。app は標準入出力で対話し続けるため、このサブセットだけがプロセスの状態と権限の問い合わせを持つ。台帳は外部起動と同じ形で書かれる(entrypoint は sdk-cli。レポート §4.2)ので台帳だけでは区別できず、app が自分の子プロセスの PID を知っていることで区分する。#361 のガード(実行中の検知)では、app は自分が起動した PID を除外する(レポート §4.4)。プロセス状態は 起動中 / 待機 / 実行中 / 権限待ち / 終了(レポート §7.1)で、台帳の状態(idle / busy)より細かい。app は起動するときに「このリポジトリの、このブランチの worktree」を用意して、そこを cwd にする(Phase 3)。どの worktree で起動したかは cwd からの推測ではなく記録された事実になるため、ワーキングツリーへ ワーキングツリーID(R) + リポジトリパス(R) で結ぶ(ワーキングツリーの個体指定子がこの2つの組であり、リポジトリパスは worktree を指す鍵の一部として持つ)。リポジトリは worktree から決まるので Gitリポジトリへは直接結ばない。git ではリポジトリ本体のディレクトリも1つ目の worktree なので、worktree を作らずに本体で起動する場合もこの関係で表せる。外部起動は cwd しか無いため、ワーキングツリーともリポジトリとも結ばない。現在のモデル・現在の権限モードは set_model / set_permission_mode で途中から変わる値で、画面に出して切り替える(レポート §6.2)。更新日時は持たない。変更の履歴はログ側(model_changed 行、ユーザー行の権限モード)に残るため。",
...
const RELATIONSHIP_DEFS: RelationshipDef[] = [
  ...
  // サブエージェント0件以上。ただし app が新しいセッションを起こした直後は、
  // セッションIDは決まっているのに会話ファイルがまだ無い(最初の行が書かれた時点で
  // 作られる)。この窓を表せるよう「0件以上」にしている(Phase 2。イシュー #403)。
  // ファイル 1 は必ず1つの会話に属する。
  {
    from: { entity: "Session", position: 270, cardinality: 1, optionality: 1 },
    to: { entity: "SessionFile", position: 90, cardinality: 3, optionality: 0 },
  ...
  // ワーキングツリー 1 : 実行中セッション(app起動)複数または値なし。E-R。
  // app は起動するときに「このリポジトリの、このブランチの worktree」を用意して、そこを
  // cwd にする(Phase 3)。どの worktree で起動したかは記録された事実になる。リポジトリは
  // worktree から決まるので Gitリポジトリへは直接結ばない。外部起動は cwd しか無いので
  // どちらとも結ばない(冒頭の判断の記録)。
  {
    from: { entity: "GitWorktree", position: 0, cardinality: 1, optionality: 1 },
    to: {
