 ...
 *   77,087行の 97.7% が求まった)。app が起動した実行中セッションだけは別で、app が
 *   起動時にプロファイル(登録済みリポジトリ)を選ぶため、サブセット側が リポジトリパス(R)
 *   を持って結ぶ(Phase 2)。
 * - 権限の問い合わせ × AI応答行
...
const RELATIONSHIP_DEFS: RelationshipDef[] = [
  ...
  // Gitリポジトリ 1 : 実行中セッション(app起動)複数または値なし。E-R。
  // app は起動するときにプロファイル(登録済みリポジトリ)を選ぶため、どのリポジトリで
  // 起動したかを記録できる。外部起動は cwd しか無いので結ばない(冒頭の判断の記録)。
  {
    from: { entity: "GitRepository", position: 300, cardinality: 1, optionality: 1 },
    to: {
