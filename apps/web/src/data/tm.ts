 ...
 *
 * 【作業ディレクトリを立てない】
 * このアプリが指定・管理するのは Gitリポジトリ(プロファイルの repository_path)だけで、
 * 作業ディレクトリ(行や実行中セッションの cwd)は Claude Code が記録するだけのもの。
 * 管理対象ではないためモノとして立てず、cwd もモデルに含めない。
 * cwd を Gitリポジトリへの (R) に置き換えることもできない。実測では、cwd を持つ行の
 * 3分の1はリポジトリのパスではなかった(サブフォルダ11.4%、worktree 9.1%、今は
 * 存在しないフォルダ12.0%)。~/.claude/projects/ のフォルダ名も、アプリはプロファイルの
 * 対象フォルダ(selected_project_folders)としてフォルダ名のまま持つため、パスから
 * 導出する項目(フォルダ名(D))は置かない。
 *
 * 【関係の検証(モノ × モノ の網羅性)】
 * 25エンティティの全300ペアを確認した。直接の結線があるのは22ペア(23本)で、
 * 対照表・対応表とその親の10ペアは垂下(mapping)でつながる。
 * 残る268ペアのうち、以下7ペアは「語彙は存在するが今は関係を構成していない」ものであり、
 * 見落としではなく判断の記録として残す。
 *
...
const ENTITY_DEFS: EntityDef[] = [
  ...
  },
  {
    ...
    type: "RESOURCE",
    description:
      "プロダクト開発の対象リポジトリ。個体指定子はリポジトリのパス。配下に CLAUDE.md / .claude/settings.json / .claude/settings.local.json を持つ。このアプリが指定・管理するのはこのリポジトリのパス(プロファイルの repository_path)だけで、作業ディレクトリはモデルに含めない(冒頭の【作業ディレクトリを立てない】を参照)。",
  ...
  // ============ イベント ============
  {
    ...
    type: "EVENT",
    description:
      "uuid / parentUuid で親子チェーンを構成する行(user / assistant / system / attachment)。記録日時という過去の出来事の日付が帰属するためイベント。行は作業ディレクトリ(cwd)も記録するが、このアプリが扱うのは Gitリポジトリだけなので、モデルには含めない(冒頭の【作業ディレクトリを立てない】を参照)。",
