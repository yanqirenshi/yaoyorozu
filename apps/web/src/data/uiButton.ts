...
export type ButtonKind = "add" | "save" | "edit" | "select";
...
/**
 * ボタンの種類。
 * 追加・保存は画面の主要な操作として塗り、変更・選択は副次的な操作として線で描く。
...
export const BUTTON_KINDS: ButtonKindSpec[] = [
  ...
  },
  {
    ...
    emphasis: "outlined",
    usage:
      "候補の中から対象を指定する操作。フォルダやファイルを選ぶダイアログを開くときなど、対象の指定そのものを行う場面に置く。形は変更ボタンと同じで、色だけ墨(ニュートラル)にしている。",
    colors: {
      // 形は変更ボタンと同じ(線のボタン)で、金茶を墨に置き換えたもの。
      // 対象を指定するだけで内容を変えないため、色は主張しないニュートラルとする。
      // 文字だけは 墨-500(変更ボタンの 金茶-500 に対応する段階)ではなく
      // text.primary(墨-900)にしている。墨-500 は白に対して 3.56:1 で本文の
      // 基準(4.5:1)に届かず、金茶のときのように色味を優先する理由もないため。
      default: { bg: "surface.raised", fg: "text.primary", border: "墨-600" },
      hover: { bg: "墨-50", fg: "text.primary", border: "墨-700" },
      active: { bg: "墨-100", fg: "text.primary", border: "墨-800" },
      disabled: {
        bg: "surface.raised",
        fg: "text.disabled",
        border: "border.default",
    ...
    contrast:
      "墨-900 の文字は白の背景に対して 11.90:1(ホバーの 墨-50 で 11.11:1、押下中の 墨-100 で 10.17:1)で、本文の基準(4.5:1)を満たす。境界は 4.78:1 以上で、境界の基準(3:1)も満たす。",
  },
];
...
export const BUTTON_RULES = [
  {
    title: "ラベルだけで構成する",
    body: "追加・保存・変更・選択のボタンはアイコンを付けず、文字だけで操作を伝える。何を追加するかが周囲から分からない場合は「プロファイルを追加」のように対象を書く。",
  },
  {
  ...
  {
    title: "線のボタンは色で役割を分ける",
    body: "対象の内容に手を入れる操作(変更)は金茶、対象を指し示すだけの操作(選択)は墨とする。同じ形のボタンが並んだとき、色だけで「データが変わるかどうか」が読み取れるようにする。",
  },
  {
