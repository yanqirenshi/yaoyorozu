...
export const TAB_CLOSE = {
  ...
  gap: "sp-1",
  /**
   * 上下の位置。タブの高さのうち、選択中の下線(インジケータ)の高さを除いた領域で
   * 上下中央に置く。× が下線の帯に重ならないようにするためで、中心はタブの高さの
   * 中央より下線の高さの半分(1.5px)だけ上になる。
   * CSS では、下に下線の高さぶんのマージン(または bottom)を取ればよい。
   */
  verticalAlign:
    "選択中の下線(インジケータ)の高さを除いた領域で上下中央。中心はタブの中央より 1.5px 上になる。",
  /** 線の色。タブの文字色を継承する。 */
  color: "inherit",
  /** 背景(× 自身の状態)。 */
  bg: {
