 ...
 * (第5弾)。設定ファイル類と、セッションまわり(ログ行・入力キュー・作業ディレクトリ・
 * 実行中セッション)は次段以降。既存のクラスとそれらの関係も、相手のクラスを書く段階で
 * 足す。作業ディレクトリ × セッションファイル の対照表は TM 側でやめる方向
 * (Data セッションの作業ブランチ)なので、作業ディレクトリを書くときは見直し後の TM に従う。
 *
 * 【TM との違い・未決】
...
const DEFS: ClassDef[] = [
  ...
  },
  {
    ...
    attributes: [
      ...
      // 会話ファイル / サブエージェントのファイル。TM で種別ごとのサブセットに分けた段階で
