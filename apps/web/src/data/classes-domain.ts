 ...
 *   誘導できない」の意味)。関連の線は「たどれる向き」(起点 → 終点)で書き、実装では
 *   起点が終点を参照する。
 * - 部分の持ち主(コンポジションの全体)は1つだけにする。すでにほかのクラスが所有している
 *   相手を指したいときは、コンポジションにせず関連にし、実装では相手を所有せず ID で参照する
 *   (例: GitWorktree は GitRepository が所有する GitBranch を ID で指す)。
 *
