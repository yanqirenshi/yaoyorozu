...
const RELATIONSHIPS = [
  ...
  // 線は「部分 → 全体」の向き(◆が Session 側に付く)。部分側の多重度が隠れない辺
  // (SessionFile の上辺と右辺)から出し、Session の上辺は User からの線で使っているので
  // 左辺と下辺で受ける。どちらの線も箱を横切らず2本が交差しないよう、SessionFile は
  // Session の左下に置く(右下に置くと、右辺から左へ向かう線が自分の箱を横切る)。
