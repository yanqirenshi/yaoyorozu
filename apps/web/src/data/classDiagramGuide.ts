...
export const GUIDE_TM_RULES: GuideRule[] = [
  ...
  {
    title: "多値(MO)は値の列か、所有される部分のクラスにする",
    body: "TM の多値(親の値を (R) で継承し、1件ごとには一意でないもの)は、属性が1つだけなら親のフィールドに値の列(Vec<String> など)として持つ。属性が複数あるときは、親が所有する部分のクラスにして、コンポジションで結ぶ(例: 権限の問い合わせ．提案 → PermissionSuggestion。0..*)。",
  },
  {
    title: "区分コードの値は enumeration にする",
    body: "サブセットの区分ではない区分コード(プロセス状態、決着種別など)は、stereotype に enumeration を付けたクラスにして、値として持つ側とコンポジションで結ぶ(線は「部分 → 全体」の向き)。導出(D)の値も型は同じなので、/ を付けたフィールドと線で表す。",
  },
  {
