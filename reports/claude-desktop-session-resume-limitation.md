...
- 作成日: 2026-08-11
- 更新日: 2026-09-24(末尾の追記の版の記述を訂正。2026-09-23 に `--resume` の制約が解消していることを追記)
- 対象: `apps/native`(ブランチ `refactor/native-b-type-architecture`)
- 関連コミット: `01be28e`, `200b95f`, `d483c43`

> **追記(2026-09-23、2026-09-24 訂正)**: 本書の中心的な制約だった「claude-desktop 起源の
> セッションは `--resume <id>` で見つけられない」は、**2026-09 時点では再現しない**。
> アプリが起動する PATH 上の claude(WinGet 版 v2.1.150)で、Desktop 起源のセッションへの
> `--resume` が応答まで通ることを確認した(公式ドキュメントにも明記)。
> 本文は 2026-08-11 時点の記録としてそのまま残す。詳細は末尾の
> 「[追記: `--resume` の制約は現行版で解消](#追記2026-09-23-resume-の制約は現行版で解消)」を参照。
...
## 追記(2026-09-23): `--resume` の制約は現行版で解消

本書の結論だった「`entrypoint: "claude-desktop"` のセッションは CLI の `--resume <id>` から
見つけられない」は、**2026-09 時点では再現しない**ことを確認した(Lab)。

> **訂正(2026-09-24)**: この追記の初版は「現行版(Claude Code v2.1.260 時点)で解消」と
> 書いていたが誤りだった。実機確認に使ったのは PATH 上の WinGet 版 **v2.1.150** で
> (Desktop 同梱は v2.1.275 / v2.1.280)、初版の時点では認証切れのため応答まで確認できて
> いなかった。issue #345 の検証 B-0 で、同じ v2.1.150 で応答まで通ることを確認した。
> なお 2026-08-11 に失敗を記録したときも同じ v2.1.150 だったため、失敗から成功に変わった
> 理由(版の違いではない)は分かっていない。

### 確認内容

1. **実機(issue #345 の B-0、Lab (PoC:検証))**: PATH 上の claude(WinGet 版 v2.1.150。
   アプリの送信が起動するもの)で、休止中の claude-desktop 起源セッションに
   `claude --resume <id> --fork-session --tools "" -p --output-format json "…"` を実行したところ、
   exit 0 で、元の会話の内容を踏まえた応答が返った。fork 先は新しいファイルで、
   元のファイルは変わらなかった。本書で記録した `No conversation found with session ID: ...` は
   発生しなかった。
2. **公式ドキュメント**([sessions](https://code.claude.com/docs/en/sessions.md))に以下が明記された:
   - `claude --resume <session-id>` は**任意のディレクトリから**実行できる
     (カレントプロジェクト → git worktree → **このマシンの全プロジェクト**の順で ID を探す)
   - `-p`(headless)や Agent SDK で作ったセッションは**ピッカーと `--continue` の対象からは
     除外**されるが、**ID を直接指定すれば `--resume` できる**
   - 各入り口(Desktop / claude.ai/code / VS Code 拡張)はセッション履歴を独立管理する
3. **逆方向(ターミナル → Desktop)も公式機能がある**
   ([desktop](https://code.claude.com/docs/en/desktop.md)):
   - Desktop アプリの `/resume` = CLI セッションを一覧して Desktop で続行する
   - CLI の `/desktop` = 実行中のターミナルセッションをライブで Desktop へ引き継ぐ

### 実務上の注意(現行版でも変わらないもの)

- **実行中のセッションを別の入り口から同時に開かない**こと(同一 `.jsonl` への並行追記で
  会話が混線する)。安全に開くなら `--resume <id> --fork-session` で分岐させる
- fork せずに追記した場合でも、**元の入り口の画面には反映されない**
  (各入り口は自前の会話状態を表示するため。ファイル上は同一の会話)

### apps/native への影響

本書の「`--resume` できないため `--continue` を選んだ」という理由付けが当てはまらなく
なったため、**`send_message` は `--resume <セッションID>` ベースに変更された**
(issue #345 の PoC、PR #346。2026-09-24 マージ)。

- 送信先をIDで指定するため、「表示中のセッション ≠ フォルダの最新」でも送信できる
  (ビューアの「送信できるのは最新のセッションのみ」の制約を撤廃)
- `--continue` の誤追記を防ぐための送信前後の照合(`session_stale` / `app:warning`)は不要になり削除
- 代わりに、他のプロセスが実行中のセッションへの並行追記を防ぐため、
  `~/.claude/sessions/<PID>.json` による実行中セッションのガードを追加
- 注意: `~/.claude/sessions/` の `procStart` は claude の版で形式が違う
  (v2.1.280 は FILETIME、v2.1.150 は .NET 形式のローカル時刻の tick 数)。
  また v2.1.150 の `startedAt` はプロセスの起動時刻ではない
