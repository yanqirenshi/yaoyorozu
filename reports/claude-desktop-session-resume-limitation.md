...
- 作成日: 2026-08-11
- 更新日: 2026-09-23(末尾に追記: `--resume` の制約は現行版で解消)
- 対象: `apps/native`(ブランチ `refactor/native-b-type-architecture`)
- 関連コミット: `01be28e`, `200b95f`, `d483c43`

> **追記(2026-09-23)**: 本書の中心的な制約だった「claude-desktop 起源のセッションは
> `--resume <id>` で見つけられない」は、**現行版(Claude Code v2.1.260 時点)では
> 解消されている**(公式ドキュメントにも明記)。本文は 2026-08-11 時点
> (v2.1.150 前後)の記録としてそのまま残す。詳細は末尾の
...
見つけられない」は、**現行版では解消されている**ことを確認した(Lab、Claude Code v2.1.260 時点)。

### 確認内容

1. **実機**: 休止中の claude-desktop 起源セッションに対し、ターミナルから
   `claude --resume <id> --fork-session --tools "" -p "…"` を実行したところ、
   本書で記録した `No conversation found with session ID: ...` は**発生しなかった**
   (セッションの発見・読み込み段階を通過。この検証時は端末側の Anthropic OAuth
   トークン失効という別問題で応答まで到達しなかったが、セッション解決の挙動確認としては十分)。
...
`send_message` の `--continue` ベース設計(表示中=最新セッションへの追記)は、
「最新の会話に追記する」という意味論が要件に合致しているため**引き続き有効**。
ただし本書の「`--resume` できないため `--continue` を選んだ」という理由付けは現行では
当てはまらず、**セッションIDを直接指定する `--resume` ベースの設計も選択肢に戻った**
(「表示中セッション ≠ 最新」の場合にも送信できる設計への拡張余地。採否はデザイン判断)。
